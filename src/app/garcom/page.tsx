...
import { Bell, Check, Wine, ReceiptText } from 'lucide-react';

export default function GarcomPage() {
  const [prontos, setProntos] = useState<any[]>([]);
  const [pedidosMesa9, setPedidosMesa9] = useState<any[]>([]);
  ...
  const carregarDados = async () => {
    ...
    // 2. Carrega todos os itens da Mesa 9 para exibição da conta
    const { data: todosMesa } = await supabase
      .from('pedidos_itens')
      .select(`
    ...
    if (todosMesa) setPedidosMesa9(todosMesa);
  };
  ...
  // Agrupa pedidos por cliente para a conta dividida
  const clientesMap = pedidosMesa9.reduce((acc: any, ped) => {
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-100 p-4 sm:p-6 pb-20">
      <header className="flex justify-between items-center border-b border-zinc-800 pb-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          >
            <ReceiptText className="w-3.5 h-3.5" /> Conta Mesa 09
          </button>
        </div>
      </header>
      ...
      {abaAtiva === 'entregas' ? (
            ...
            <div key={item.id} className="bg-zinc-900 border border-emerald-500/40 rounded-xl p-4 flex justify-between items-center">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-extrabold text-emerald-400 text-lg">MESA {item.comandas_mesa?.mesas?.numero || '09'}</span>
        ...
        /* Fechamento Consolidado da Mesa com Rateio */
        <div className="max-w-xl mx-auto bg-white text-zinc-900 rounded-xl p-6 shadow-xl font-mono text-xs">
          <div className="text-center pb-4 border-b border-zinc-200">
            <h2 className="text-xl font-black">CANTINA DO JÃO KIM</h2>
            <p className="text-zinc-500">FECHAMENTO DA MESA 09</p>
          </div>
